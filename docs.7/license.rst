...
version. If need use the latest version, purchasing a new license, and run
`pyarmor register` command. If you have issued any `license.lic` generated by
old license, and still want it works, run `pyarmor register` with option `-u` to
register new license. For example,
