...
* From pyarmor v6.5.0, run the following command to register PyArmor
  ...
  The machine must be connected to internet in order to register PyArmor.

* Before pyarmor v6.5.0, first open the following url in any web browser to
  activate this code:

      https://api.dashingsoft.com/product/key/activate/CODE/

  A registration keyfile generally named "pyarmor-regfile-1.zip" will be
  downloaded, then run the following command to register PyArmor

      pyarmor register /path/to/pyarmor-regfile-1.zip

After registration successfully, check the registeration information:

    pyarmor register

Then obfuscate all the scripts again.
