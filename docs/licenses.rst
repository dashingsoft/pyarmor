...
.. important::
    ...
    **Build Device** is used to generate the obfuscated scripts, Pyarmor and Pyarmor License is only installed in Build Device

    It need install neither Pyarmor nor Pyarmor License to execute obfuscated scripts in target device, so there is no device limitations for target device
