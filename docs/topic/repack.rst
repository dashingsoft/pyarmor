...
If no this option, pyarmor only generates obfuscated scripts.

If this option is set, pyarmor does extra works:

* Unpacking this executable to a temporary folder
* Replacing the python scripts with obfuscated ones
* Appedning runtime files to the bundle in this temporary folder
* Repacking this temporary folder to patched executable
* Move patched executable to overwite the original executable
...
* First obfuscating the scripts by Pyarmor [#]_::

    cd /path/to/src
    pyarmor gen -O obfdist foo.py

* Moving runtime package to current path [#]_::

    mv obfdist/pyarmor_runtime_000000 ./

* Already have ``foo.spec``, appending runtiem package to ``HIDDEN_IMPORTS``

.. code-block:: python

    HIDDEN_IMPORTS = [..., 'pyarmor_runtime_000000']
