...
class Configer(object):
    ...
    def run(self, section=None, option=None, value=None, local=0):
        cfg = configparser.ConfigParser(empty_lines_in_values=False)
        cfg.read(self.ctx.local_config if local else self.ctx.global_config)
        if section is None:
            print('%s sections:' % ('local' if local else 'global'))
            print('\n'.join(cfg.sections()))
            return

        if not cfg.has_section(section):
            print('no found section %s' % section)

        if option is None:
            print('options in section %s:' % section)
            print('\n'.join(cfg[section].options()))
            return
