...
@logaction
def run_setup_script(src, entry, setup, packcmd, prodist):
...
setup script to build the bundle.
    ...
    new_entry = os.path.join(prodist, entry)
...
def _packer(src, entry, setup, packcmd, output, libname):
    ...
    run_setup_script(src, entry, setup, packcmd, prodist)

    update_library(prodist, os.path.join(output, libname))
