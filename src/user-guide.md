...
* `license.lic` of Pyarmor, which locates in the source path of pyarmor

* `license.lic` of Obfuscated Scripts, which is generated when
  obfuscating a project or by pyarmor command `licenses`

Each project has its own capsule `.pyarmor_capsule.zip` in project
path, when run command `pyarmor build` or `pyarmor licenses`, the
`license.lic` of Obfuscated Scripts will be generated by this capsule
`.pyarmor_capsule.zip`.

The project capsule is generated when run command `pyarmor init`. So
